...
class Carro(Veiculo):
    ...
    def desligar(self):
        if self.desligar == False:
            print('Veiculos desligado.')
        else:
...
moto_objeto = Moto("Honda", "CG", False)
carro_objeto = Carro('Fiat', 'Pailo', False)
...
garagem_objeto.estacionar(f'Carro da pai {carro_objeto.marca}')
garagem_objeto.estacionar(f'Moto da mãe {moto_objeto.marca}')
garagem_objeto.estacionar(f'Moto da mãe {moto_objeto.modelo}')
